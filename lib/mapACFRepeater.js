const mapACFRepeater = (data) => {
    const items = [];
    const numOfItems = data.item >=0 && data.item || data.items >=0 && data.items || data.checklist_descriptions >=0 && data.checklist_descriptions || data.title >=0 && data.title || data.checklist >=0 && data.checklist || data.featuresarray >=0 && data.featuresarray || 0;
